...
def run_trial(sarsaLearner, descriptor="", maxEpisodeCount=500, maxEpisodeLength=500, show_results=False):
    ...
    if show_results:
        plot_line(cumulative_episode_lengths, range(0, maxEpisodeCount+1), "Timesteps", "Episode Counts", f"SARSA with ET - Timesteps vs Episode Counts ({descriptor})", show_results)
        sarsaLearner.slidingMap.simulateTrajectory(100, 2500)
    return cumulative_episode_lengths, deviation_over_episodes

# sarsaLearner = SARSAEligibilityTraces()
# run_trial(sarsaLearner, "basic", show_results=True)

# sarsaLearner = SARSAEligibilityTraces(4, 4, 2)
# run_trial(sarsaLearner, "basic - large and simple", maxEpisodeCount=800, show_results=True)

# sarsaLearner = SARSAEligibilityTraces(num_crates=4)
...
# sarsaLearner = SARSAEligibilityTraces(obstacles={'o1': (1, 1)})
...
# compare_lines_lat(cumulative_episode_lengths_by_alpha, range(0, MAX_EPISODES_COUNTS+1), "Timesteps", "Episode Counts", "SARSA with ET - Timesteps vs Episode Counts - Obstacle - Alpha Variations", alpha_candidates, False)
...
#     for sample_idx in range(0, MAX_SAMPLES):
#         print(f'[Sample {sample_idx}]')
#         sarsaLearner = SARSAEligibilityTraces(obstacles={'o1': (1, 1)}, default_q_value=default_q_vals_candidates[trial_idx])
#         cumulative_episode_lengths, deviation_over_episodes = run_trial(sarsaLearner, "basic - obstacle", maxEpisodeCount=MAX_EPISODES_COUNTS)
#         cumulative_episode_lengths_by_sample.append(cumulative_episode_lengths)
#     mean_cumulative_episode_lengths = []
#     for episode_idx in range(0, MAX_EPISODES_COUNTS+1):
...
# compare_lines_lat(cumulative_episode_lengths_by_q_vals, range(0, MAX_EPISODES_COUNTS+1), "Timesteps", "Episode Counts", "SARSA with ET - Timesteps vs Episode Counts - Obstacle - Default Q Val Variations", default_q_vals_candidates, False)
...
# compare_lines_lat(cumulative_episode_lengths_by_reward_spec, range(0, MAX_EPISODES_COUNTS+1), "Timesteps", "Episode Counts", "SARSA with ET - Timesteps vs Episode Counts - Obstacle - Reward Spec Variations", [f"{spec[0]}_{spec[1]}" for spec in reward_spec_candidates], False)
...
# compare_lines_lat(cumulative_episode_lengths_by_discount_param, range(0, MAX_EPISODES_COUNTS+1), "Timesteps", "Episode Counts", "SARSA with ET - Timesteps vs Episode Counts - Obstacle - Discount Param Variations", discount_param_candidates, False)


# # Basic Obstacle Experiment ##
# MAX_SAMPLES = 4
# MAX_EPISODES_COUNTS = 300
...
# plot_line(mean_cumulative_episode_lengths, range(0, MAX_EPISODES_COUNTS), "Timesteps", "Episode Counts", "SARSA with ET - Cumulative timesteps vs Episode Counts - Obstacle", False)
# plot_line(range(1, MAX_EPISODES_COUNTS), mean_deviation_between_samples, "Episode Counts", "MSE Difference from previous epsiode q values", "SARSA with ET - MSE difference from previous Q Values - Obstacle", False)
